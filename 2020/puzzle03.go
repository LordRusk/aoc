// this was made in the golang playground because
// my computer got taken for not having a -B average
// during zoom infested ronachan.
package main
...
func isValidPass(pass string) bool {
	...
	splitPass = strings.Split(splitPass[0], "-")
	lReq, _ := strconv.Atoi(splitPass[0])
	hReq, _ := strconv.Atoi(splitPass[1])
	...
	var numReqLet int
	for scanner.Scan() {
		if scanner.Text() == reqLet {
			numReqLet++
		}
	}

	if numReqLet >= lReq && numReqLet <= hReq {
		return true
	}
