...
func main() {
	sData := strings.Split(pu, "\n\n")
	pData := make([][][]string, len(sData))

	for num, group := range sData {
		sGroup := strings.Split(group, "\n")
		pData[num] = make([][]string, len(sGroup))

		for numm, person := range sGroup {
			for _, a := range strings.Split(person, "") {
				pData[num][numm] = append(pData[num][numm], a)
			}
		}
	}

	var yesd int
	for _, group := range pData {
		yesdMap := make(map[string]int, len(group))
		...
		for _, value := range yesdMap {
			if value == len(group) {
				yesd++
			}
		}

		fmt.Printf("map for %v is %v\n", group, yesdMap) // visualization
	}
