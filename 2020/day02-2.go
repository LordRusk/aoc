// this was made in the golang playground on a chromebook
// because my computer got taken for not having a -B
// average during zoom infested ronachan.
package main
...
func main() {
	...
	var numValidPass int
	for scanner.Scan() {
		if isValidPass(scanner.Text()) {
			numValidPass++
		}
	}

	fmt.Printf("Puzzle input is: %v\n", numValidPass)
}
...
func isValidPass(pass string) bool {
	...
	splitPass = strings.Split(splitPass[0], " ")
	reqLet := splitPass[1]

	splitPass = strings.Split(splitPass[0], "-")
	pos1, _ := strconv.Atoi(splitPass[0])
	pos2, _ := strconv.Atoi(splitPass[1])

	scanner := bufio.NewScanner(strings.NewReader(pass))
	scanner.Split(bufio.ScanBytes)

	var isPos1, isPos2 bool
	var passovers int
	for scanner.Scan() {
		passovers++

		if passovers == pos1 && scanner.Text() == reqLet {
			isPos1 = true
		} else if passovers == pos2 && scanner.Text() == reqLet {
			isPos2 = true
		}
	}
