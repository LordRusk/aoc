...
func main() {
	var pData [][]string
	for _, group := range strings.Split(pu, "\n\n") {
		pData = append(pData, strings.Split(strings.ReplaceAll(group, "\n", ""), ""))
	}

	var yesd int
	for _, group := range pData {
		yesMap := make(map[string]bool, len(group))

		for _, a := range group {
			yesMap[a] = true
		}

		for _, a := range yesMap {
			if a {
				yesd++
			}
		}
	...
	fmt.Printf("Number of questions answered yes: %v\n", yesd)
}
